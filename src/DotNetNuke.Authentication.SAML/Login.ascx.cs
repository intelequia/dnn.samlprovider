		...
		protected override void OnLoad(EventArgs e)
        {
            ...
            try
            {
                ...
                if (Request.HttpMethod == "POST")
                {
                    ...
                    if (loginStatus == UserLoginStatus.LOGIN_SUCCESS)
                    {
                        ...
                        if (!string.IsNullOrEmpty(Request["RelayState"]))
                        {
                            ...
                            var relayStateRet = relayStateList.FirstOrDefault(x => x.ToLowerInvariant().StartsWith("rur="));
                            if (relayStateRet != null)
                            {
                                redirectUrl = HttpUtility.UrlDecode(relayStateRet.Substring(relayStateRet.IndexOf("rur=") + 4));
                            }
                        }
                ...
                else
                {
                    ...
                    {
                        relayState = HttpUtility.UrlEncode("rur=" + HttpUtility.UrlDecode(Request.QueryString["returnurl"]));
                    }
